...
Script de configuração de repositório Python com uv, hooks de qualidade de código e branch protection.
Deve ser executado dentro do repo já criado e clonado via `gh repo create`.
...
| `check-pr-direction.yml` | Bloqueia PR para master que não venha de qa |
...
PR direction    →  check-pr-direction.yml        (bloqueia PR para master fora de qa)
```

> Os hooks locais podem ser burlados com `--no-verify`. O GitHub Actions é a camada que não tem bypass local.
